...
-- Tiempo de generación: 13-11-2018 a las 18:58:08
...
DROP PROCEDURE IF EXISTS `SP_GESTION_RECUPERAR_INDICADORES2`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_GESTION_RECUPERAR_INDICADORES2` (IN `inicio` DATE, IN `fin` DATE, IN `idProy` INT(11))  NO SQL
BEGIN
...
IFNULL((SELECT SUM(tg.DiasGestion) FROM actividad ac3 INNER JOIN tarea ta3 On ta3.Actividad_idActividad=ac3.idActividad INNER JOIN tareagestion tg On tg.Tarea_idTarea=ta3.idTarea where ac3.Proyecto_idProyecto=pro.idProyecto and (tg.fechaInicio BETWEEN inicio and fin OR tg.fechaFin BETWEEN inicio and fin)),'0')as HorasRealizadas,

(SELECT SUM(ac5.Costo) FROM actividad ac5 WHERE ac5.Proyecto_idProyecto=pro.idProyecto) as CostoPresupuestado,

IFNULL(((SELECT SUM(tg.DiasGestion) FROM actividad ac3 INNER JOIN tarea ta3 On ta3.Actividad_idActividad=ac3.idActividad INNER JOIN tareagestion tg On tg.Tarea_idTarea=ta3.idTarea where ac3.Proyecto_idProyecto=pro.idProyecto and (tg.fechaInicio BETWEEN inicio and fin OR tg.fechaFin BETWEEN inicio and fin))*100)/(TIMESTAMPDIFF(DAY,pro.fechaInicio,pro.fechaFin)),'0') as PorcentajeAvance,

IFNULL(100-((SELECT SUM(tg.DiasGestion) FROM actividad ac3 INNER JOIN tarea ta3 On ta3.Actividad_idActividad=ac3.idActividad INNER JOIN tareagestion tg On tg.Tarea_idTarea=ta3.idTarea where ac3.Proyecto_idProyecto=pro.idProyecto and (tg.fechaInicio BETWEEN inicio and fin OR tg.fechaFin BETWEEN inicio and fin))*100)/(TIMESTAMPDIFF(DAY,pro.fechaInicio,pro.fechaFin)),'0') as PorcentajeNoAvance

FROM proyecto pro LEFT JOIN actividad ac ON ac.Proyecto_idProyecto=pro.idProyecto
WHERE pro.idProyecto=idProy
GROUP BY pro.idProyecto;
 

END$$

DROP PROCEDURE IF EXISTS `SP_GESTION_RECUPERAR_INDICADORES3`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_GESTION_RECUPERAR_INDICADORES3` (IN `inicio` DATE, IN `fin` DATE, IN `idAct` INT(11))  NO SQL
BEGIN

SELECT  
(SELECT COUNT(DISTINCT(par.Persona_idPersona)) FROM participacion par INNER JOIN actividad act2 ON act2.idActividad=par.Actividad_idActividad where act2.idActividad=ac.idActividad) as CantidadParticipantes,

(SELECT COUNT(*) FROM actividad ac1 where ac1.idActividad=ac.idActividad) as CantidadActividades,

(SELECT COUNT(*) FROM actividad ac2 inner join tarea ta2 ON ta2.Actividad_idActividad=ac2.idActividad where ac2.idActividad=ac.idActividad) as CantidadTareas,

(TIMESTAMPDIFF(DAY,ac.fechaInicio,ac.fechaFin)) as HorasProgramadas,

IFNULL((SELECT SUM(tg.DiasGestion) FROM actividad ac3 INNER JOIN tarea ta3 On ta3.Actividad_idActividad=ac3.idActividad INNER JOIN tareagestion tg On tg.Tarea_idTarea=ta3.idTarea where ac3.idActividad=ac.idActividad and (tg.fechaInicio BETWEEN inicio and fin OR tg.fechaFin BETWEEN inicio and fin)),'0')as HorasRealizadas,

ac.Costo as CostoPresupuestado,

IFNULL(((SELECT SUM(tg.DiasGestion) FROM actividad ac3 INNER JOIN tarea ta3 On ta3.Actividad_idActividad=ac3.idActividad INNER JOIN tareagestion tg On tg.Tarea_idTarea=ta3.idTarea where ac3.idActividad=ac.idActividad and (tg.fechaInicio BETWEEN inicio and fin OR tg.fechaFin BETWEEN inicio and fin))*100)/(TIMESTAMPDIFF(DAY,ac.fechaInicio,ac.fechaFin)),'0') as PorcentajeAvance,

IFNULL(100-((SELECT SUM(tg.DiasGestion) FROM actividad ac3 INNER JOIN tarea ta3 On ta3.Actividad_idActividad=ac3.idActividad INNER JOIN tareagestion tg On tg.Tarea_idTarea=ta3.idTarea where ac3.idActividad=ac.idActividad and (tg.fechaInicio BETWEEN inicio and fin OR tg.fechaFin BETWEEN inicio and fin))*100)/(TIMESTAMPDIFF(DAY,ac.fechaInicio,ac.fechaFin)),'0') as PorcentajeNoAvance

FROM  actividad ac 
WHERE ac.idActividad=idAct;
 

END$$

DROP PROCEDURE IF EXISTS `SP_GESTION_RECUPERAR_INFORMACION_TAREA`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_GESTION_RECUPERAR_INFORMACION_TAREA` (IN `idTareaU` INT(11))  NO SQL
BEGIN 

DECLARE NombreTarea VARCHAR(150);
DECLARE Descripcion TEXT;
DECLARE fechaInicio DATE;
DECLARE fechaFin DATE;
DECLARE fechaRegistro DATETIME;
DECLARE idTarea INT(11);
DECLARE diasT INT(11);
DECLARE Avance VARCHAR(100);

SET NombreTarea=(SELECT t.NombreTarea FROM tarea t WHERE t.idTarea=idTareaU);
SET Descripcion=(SELECT t.Descripcion FROM tarea t WHERE t.idTarea=idTareaU);
SET fechaInicio=(SELECT t.fechaInicio FROM tarea t WHERE t.idTarea=idTareaU);
SET fechaFin=(SELECT t.fechaFin FROM tarea t WHERE t.idTarea=idTareaU);
SET fechaRegistro=(SELECT t.fechaRegistro FROM tarea t WHERE t.idTarea=idTareaU);
SET idTarea=(SELECT t.idTarea FROM tarea t WHERE t.idTarea=idTareaU);
SET diasT=(SELECT TIMESTAMPDIFF(DAY,fechaInicio,fechaFin) FROM  tarea t WHERE t.idTarea=idTareaU);

SET Avance=(SELECT
CONCAT(FORMAT(IFNULL((
    (SELECT SUM(tg.DiasGestion) FROM actividad ac3 INNER JOIN tarea ta3 On ta3.Actividad_idActividad=ac3.idActividad INNER JOIN tareagestion tg On tg.Tarea_idTarea=ta3.idTarea where ta3.idTarea=t.idTarea)  *100)/(TIMESTAMPDIFF(DAY,t.fechaInicio,t.fechaFin)),'0.00'),2),'%') as TareadAvance
FROM tarea t WHERE t.idTarea=idTareaU);

SELECT NombreTarea,Descripcion,fechaInicio,fechaFin,fechaRegistro,idTarea,diasT,Avance;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_RECUPERAR_INDICADOR_FECHAS` (IN `FechaEnviada` DATE)  NO SQL
BEGIN 

SELECT
ac.NombreTarea,
ac.Costo,
(SELECT SUM(tg.DiasGestion) 
 FROM actividad ax 
 INNER JOIN tarea ta
 On ta.Actividad_idActividad=ax.idActividad  
 INNER JOIN tareagestion tg 
 ON tg.Tarea_idTarea=ta.idTarea
 where ax.idActividad=ac.idActividad and (FechaEnviada BETWEEN tg.fechaInicio AND tg.fechaFin) ) as Avance
 
FROM proyecto pro INNER JOIN actividad ac On pro.idProyecto=ac.Proyecto_idProyecto 
where pro.idProyecto=5;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_REPORTE_1` (IN `Inicio` DATE, IN `Fin` DATE, IN `idProy` INT(11))  NO SQL
BEGIN 
DECLARE done INT DEFAULT FALSE;
DECLARE v_idAs BIGINT;
DECLARE diaTemporal DATE;
 
DECLARE CursorReporte CURSOR FOR 
SELECT ac.idActividad FROM actividad ac INNER JOIN proyecto pro On pro.idProyecto=ac.Proyecto_idProyecto where pro.idProyecto=idProy;

DECLARE CONTINUE HANDLER FOR NOT FOUND SET done = TRUE;

CREATE TEMPORARY TABLE TablaTemporal (campo1 VARCHAR(150),campo2 DATE, 
campo3 DECIMAL(10,2), 
campo4 DECIMAL(10,2),
campo5 DECIMAL(10,2)                                      
);           
...
FETCH CursorReporte into v_idAs;
...
SET @NombreActividad=(SELECT ac.NombreTarea FROM actividad ac 
      WHERE ac.idActividad=v_idAs);
      
SET @CostoPresupuestado=(SELECT ac.Costo as CostoPresupuestado FROM actividad ac 
WHERE ac.idActividad=v_idAs);

SET @PorcentajeAvance=(SELECT IFNULL(((
    SELECT SUM(tg.DiasGestion) 
    FROM actividad ac3 INNER JOIN tarea ta3 On ta3.Actividad_idActividad=ac3.idActividad 
    INNER JOIN tareagestion tg On tg.Tarea_idTarea=ta3.idTarea where ac3.idActividad=ac.idActividad and (tg.fechaInicio >= diaTemporal OR diaTemporal<=tg.fechaFin ))*100)/(TIMESTAMPDIFF(DAY,ac.fechaInicio,ac.fechaFin)),'0') FROM  actividad ac 
WHERE ac.idActividad=v_idAs);

SET @PorcentajeNoAvance=(SELECT IFNULL(100-((SELECT SUM(tg.DiasGestion) FROM actividad ac3 INNER JOIN tarea ta3 On ta3.Actividad_idActividad=ac3.idActividad INNER JOIN tareagestion tg On tg.Tarea_idTarea=ta3.idTarea where ac3.idActividad=ac.idActividad and (tg.fechaInicio >= diaTemporal OR diaTemporal<=tg.fechaFin))*100)/(TIMESTAMPDIFF(DAY,ac.fechaInicio,ac.fechaFin)),'0') FROM actividad ac 
WHERE ac.idActividad=v_idAs );    
 
INSERT INTO TablaTemporal(campo1,campo2,campo3,campo4,campo5) VALUES (@NombreActividad,diaTemporal,@CostoPresupuestado,@PorcentajeAvance,@PorcentajeNoAvance); 
...
SELECT * FROM TablaTemporal ORDER BY campo2 ASC;
END$$

DROP PROCEDURE IF EXISTS `SP_SUBTAREA_ACTUALIZAR`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_SUBTAREA_ACTUALIZAR` (IN `idTareaU` INT(11), IN `DescripcionU` TEXT, IN `Inicio` DATE, IN `Fin` DATE, IN `estado` INT(11), IN `idActividadU` INT(11), IN `idProyectoU` INT(11), IN `creador` INT(11), IN `Nombre` VARCHAR(150))  NO SQL
BEGIN
...
INSERT INTO `bitacora`(`idBitacora`, `usuarioAccion`, `Accion`, `tablaAccion`,`Detalle`, `fechaRegistro`) VALUES (null,@NombreUsuario,'ACTUALIZACION','SUBTAREA',CONCAt("SE ACTUALIZO SUBTAREA:",DescripcionU),NOW());
END$$
...
INSERT INTO `bitacora`(`usuarioAccion`, `Accion`, `tablaAccion`,`Detalle`, `fechaRegistro`) VALUES (@usuario,@Mensaje,'SUBTAREA',@Mensaje,NOW());
...
(15, 'Cierre2', 'Fase de Cierre del Proyecto', '18000.00', '2018-10-20 17:19:12', '2018-12-31', '2019-01-19', 5, NULL, 3),
...
(589, 'ADMINISTRADOR GENERAL DEL SISTEMA', 'INSERTAR', 'SE REGISTRO TAREA', 'TAREA', '2018-10-24 13:25:55'),
...
(594, 'ADMINISTRADOR GENERAL DEL SISTEMA', 'INSERTAR', 'SE REGISTRO ACTIVIDAD', 'ACTIVIDAD', '2018-11-13 13:40:56'),
(595, 'ADMINISTRADOR GENERAL DEL SISTEMA', 'INSERTAR', 'SE REGISTRO ACTIVIDAD', 'ACTIVIDAD', '2018-11-13 13:44:27'),
(596, 'admin', 'ACTUALIZACION', 'ACTIVIDAD', 'SE ACTUALIZO ACTIVIDAD:Cierre23', '2018-11-13 13:44:43'),
(597, 'admin', 'ACTUALIZACION', 'ACTIVIDAD', 'SE ACTUALIZO ACTIVIDAD:Cierre2', '2018-11-13 13:45:12'),
(598, 'admin', 'ACTUALIZACION', 'ACTIVIDAD', 'SE ACTUALIZO ACTIVIDAD:Cierre', '2018-11-13 13:45:19'),
(599, 'admin', 'ACTUALIZACION', 'ACTIVIDAD', 'SE ACTUALIZO ACTIVIDAD:Cierre2', '2018-11-13 13:47:03'),
(600, 'admin', 'ACTUALIZACION', 'ACTIVIDAD', 'SE ACTUALIZO ACTIVIDAD:Cierre2', '2018-11-13 13:47:37'),
(601, 'admin', 'SUBTAREA HABILITADO', 'SUBTAREA', 'SUBTAREA HABILITADO', '2018-11-13 13:51:19'),
(602, 'admin', 'SUBTAREA DESHABILITADO', 'SUBTAREA', 'SUBTAREA DESHABILITADO', '2018-11-13 13:51:33'),
(603, 'admin', 'SUBTAREA HABILITADO', 'SUBTAREA', 'SUBTAREA HABILITADO', '2018-11-13 13:51:35'),
(604, 'admin', 'SUBTAREA DESHABILITADO', 'SUBTAREA', 'SUBTAREA DESHABILITADO', '2018-11-13 13:51:41'),
(605, 'admin', 'SUBTAREA HABILITADO', 'SUBTAREA', 'SUBTAREA HABILITADO', '2018-11-13 13:52:52'),
(606, 'admin', 'SUBTAREA DESHABILITADO', 'SUBTAREA', 'SUBTAREA DESHABILITADO', '2018-11-13 13:52:59'),
(607, 'admin', 'SUBTAREA HABILITADO', 'SUBTAREA', 'SUBTAREA HABILITADO', '2018-11-13 13:53:03');
...
(1, 1, 'admin', '$2a$08$RCuzW/8g2Lg4QMNCfmsa/uKp33rvDmdWrC.P40DOECJlMtPu16NMW', 'Administrador', '2018-09-29 14:03:44', '::1', '2018-11-13 13:55:49');
...
(5, 'Sistema WEB para el Proceso de Producción', 5, 'Sistema de Producción para el CLiente INCASOFT SAC', '2018-10-20 15:19:38', '2018-10-01', '2018-11-10', 40, 1),
...
(18, 'Tarea 2', 'Tarea 2', '2018-10-21 21:48:00', '2019-01-11', '2019-01-19', NULL, 15, 1),
...
(43, 'TAREA 1', 'TAREA 1', '2018-10-21 22:34:54', '2018-11-30', '2018-12-13', NULL, 30, 6),
...
(12, 30, 45, 3, 3, '2018-10-01', '2018-10-04', 'wfwefwe', '2018-10-23'),
(13, 31, 45, 7, 7, '2018-10-04', '2018-10-11', 'fsdf', '2018-10-23'),
(14, 32, 45, 14, 5, '2018-10-11', '2018-10-16', 'wfwefwef', '2018-10-23');
